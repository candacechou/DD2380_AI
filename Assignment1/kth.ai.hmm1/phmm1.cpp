...
void Update_alpha(float**& alpha,float**& A, float**& B, int Ot,int t);
...
int main (int argc, char *argv[]){
    ...
    Initial_Matrix(alpha,obser_time,num_state);
    ...
    for(int t = 1;t<obser_time ; t++){
    	Ot = observation[t];
    	Update_alpha(alpha,A,B,Ot,t);
    	// print out to check
    // 	cout << "the "<< t << "timestep"<<endl;
    // 	for(int i = 0;i<num_state;i++){
    // 		cout<<alpha[t][i]<<" ";
    // 	}
    // 	cout<<endl;
    ...
    for(int i = 0;i < num_state;i++){
    	...
    	// cout<<P_o<<" "<<alpha[obser_time-1][i]<<endl;
